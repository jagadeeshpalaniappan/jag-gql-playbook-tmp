...
class ProjectsSideNav extends React.Component<Props & ReduxProps, {}> {
  render() {
    ...
    const { config, onNewWorkspace, isElectron } = this.props;

    const folderName = "My App1";
    const endpoints = {
      dev: "http://localhost:3333/demo/graphql/v1/api",
      qa: "http://localhost:3333/demo/graphql/v1/api",
    };
    const projects = {
      Proj1: {
        extensions: {
          endpoints: { ep1: "http://localhost:3333/demo/graphql/v1/api" },
        },
      },
    };
    return (
